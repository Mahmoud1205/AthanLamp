...
## Time spent so far
The PCB is not fully assembled yet, but so far I spent approximately a month of work designing the hardware, 5 days of PCB fabrication attempts where each day took 6-8 hours of work at the makerspace, and 2 days of soldering which also took around 6-8 hours each. I don't remember when I started this project exactly but I can approximate it to around the middle of March.

## Planning and designing the hardware
...
* Using the SMD version of ATmega328P instead of THT. I only did this because it looked nicer and it saved space on the PCB to fit inside the mosque, but this decision caused me big problems during the soldering stage, the footprint is too small to easily solder by hand, but I will try to fix this soon, maybe by using hot air gun to solder.
...
* Using 16MHz crystal oscillator for the MCU. The ATmega328P spec does not officially support this clock speed at 3.3V but I found many people online who say it works even if it's not officially supported so I will take the risk. I took this risk because I want to compress the athan audios using ADPCM and I need a high clock speed to decompress and output PWM in real time.
...
* Limiting the PCB to 2 layers only because I will fabricate the PCB by myself using a CNC machine where middle layers are not possible.
...
* I didn't want to spend time I spent a little bit more than a week trying to fabricate the PCB and I kept failing each time until it worked, and in the end I still had to use a bodge wire because one of the traces on the PCB was discontinuous because it's too thin <sup>(see the photo below)</sup>.<br>
<img alt="bodge_wire" src="../Media/bodge_wire.jpeg" />

You can see all the failed PCB attempts below, each one took a whole day of work at the makerspace.<br>
...
## Soldering
I am stuck on this part because I have exams soon so I don't have enough time to go to the makerspace and solder the board. But I will continue after the exams immediately. But the progress I have made so far is that I soldered the voltage regulation circuits and tested them with a multimeter. All three voltage rails show the correct voltages: 12V, 5V, 3.3V. And the 3 indicator lights for each power rail light up. To reach this stage it took me approximately 3 days. I will update this part when continue the project after my exams.
